...
class ZeROStage3(Scene):
    def construct(self):
        ...
        header = MarkupText("ZeRO Stage 3:\nFull Parameter Partitioning", font_size=20, weight=BOLD, font="Arial").to_corner(UL)
        self.add(header)

        def make_state(val): return [[val]*4 for _ in range(6)]
        
        state_w = make_state(1) # Start completely sharded
        state_g = make_state(0) # Empty
        state_o = make_state(1) # Sharded
        ...
        # --- 1: Forward (Layer by Layer) ---
        for L in range(4):
            self.play(*update_center(f"Step 1: Gather Layer {L+1}", f"Gather weights for layer {L+1}"))
            for i in range(6): state_w[i][L] = 2
            gpus_current = animate_transition(gpus_current, state_w, state_g, state_o)
            self.wait(0.5)

            self.play(*update_center(f"Step 1: Forward Layer {L+1}", "Compute layer activations.\nDiscard unused weights."))
            for i in range(6): state_w[i][L] = 1
            gpus_current = animate_transition(gpus_current, state_w, state_g, state_o)
            self.wait(0.5)
